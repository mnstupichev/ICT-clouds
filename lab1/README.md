# Лабораторная 1

## Этапы выполнения

### 0. Анализ лабораторной

Спросил нашего [китайского друга](https://chat.deepseek.com)  - "Как лучше выполнять работу по шагам" и "Получиться ли выполнить работу на Windows".
Конечно же можно (я думаю так в начале отчета), так что установил WSL и настроил работу хостов.
#### Настройка hosts в Windows:
Открыл Блокнот от имени администратора

Открыл файл C:\Windows\System32\drivers\etc\hosts

Добавил строки:
```
127.0.0.1 alpha.test www.aaa.test
127.0.0.1 beta.test www.bbb.test
```
<img width="1414" height="724" alt="image" src="https://github.com/user-attachments/assets/2158f260-0522-445c-a74c-360f7112bfe5" />

Первоначальные настройки закончены, пока что все идет хорошо, настроение тоже заммечательное. :)

### 1. Подготовка структуры каталогов

Так как искать готовый пет проект было лень, я создал два небольших с парой строчек.
#### Для этого создадим папки для наших двух проектов и их логирования:
```
# Создаем корневые директории для файлов проектов
sudo mkdir -p /var/www/alpha.test/html
sudo mkdir -p /var/www/beta.test/html

# Создаем папки для логов
sudo mkdir -p /var/www/alpha.test/logs
sudo mkdir -p /var/www/beta.test/logs

# Создаем простые index.html файлы для каждого проекта
sudo nano /var/www/alpha.test/html/index.html
```
<img width="1246" height="349" alt="image" src="https://github.com/user-attachments/assets/f8b16318-496f-4a31-a55d-1c61911546d7" />

```
sudo nano /var/www/beta.test/html/index.html
```
<img width="1332" height="441" alt="image" src="https://github.com/user-attachments/assets/f6d6074a-98e4-4aac-8b77-1b28a4065571" />

```
# Устанавливаем правильные права владения
sudo chown -R www-data:www-data /var/www/alpha.test
sudo chown -R www-data:www-data /var/www/beta.test
sudo chmod -R 755 /var/www
```
<img width="972" height="276" alt="image" src="https://github.com/user-attachments/assets/3fcc557e-e1bc-4c91-ab90-cab51788e473" />

Отлично, каталоги готовы. 🎉

### 2. Генерация SSL-сертификатов (самоподписанных)

Как я понял можно взять где-то бесплатные, но сделать свои как будто бы легче, так что сгенерируем самоподписанные:

```
# Создаем директорию для сертификатов
sudo mkdir /etc/nginx/ssl
```
```
# Генерируем ключ и сертификат для alpha.test
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
    -keyout /etc/nginx/ssl/alpha.test.key \
    -out /etc/nginx/ssl/alpha.test.crt
```
<img width="1894" height="609" alt="image" src="https://github.com/user-attachments/assets/b7746013-a349-420d-9651-b9c86b097554" />

```
# Генерируем ключ и сертификат для beta.test
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
    -keyout /etc/nginx/ssl/beta.test.key \
    -out /etc/nginx/ssl/beta.test.crt
```
<img width="1904" height="717" alt="image" src="https://github.com/user-attachments/assets/cf1f4fd5-be69-4d58-83b2-1074ee447431" />

Пояснения (для себя больше):
```
-x509: Создать самоподписанный сертификат

-nodes: Не шифровать приватный ключ (чтобы Nginx мог читать его без пароля)

-days 365: Срок действия сертификата
```
Пока что не было ниодной ошибки, это успех. 🔥

### 3. Настройка виртуальных хостов (конфиги сайтов)

Трудная часть лабы. Поехали.

#### Создадим по отдельному конфигурационному файлу для каждого сайта в директории sites-available.

* Файл для alpha.test:
```
sudo nano /etc/nginx/sites-available/alpha.test
```
<img width="1339" height="917" alt="image" src="https://github.com/user-attachments/assets/9b34946d-1d79-4415-9ae5-f04875461fb9" />

* Тоже самое для beta.test:
```
sudo nano /etc/nginx/sites-available/beta.test
```
<img width="1332" height="876" alt="image" src="https://github.com/user-attachments/assets/ba992d98-8fda-45f9-bec0-a1eafddf8fcb" />

Пояснение (тоже для себя):
```
Первый блок server: Ловит все HTTP-запросы и с помощью кода 301 (Moved Permanently) перенаправляет их на тот же URL, но по протоколу HTTPS.

Второй блок server: Обрабатывает HTTPS-соединения.

Директива ssl_certificate: Указывает на наш созданный сертификат.

Директива location с alias: Это ключевой момент. Она позволяет сопоставить URL-путь с путем в файловой системе, который находится вне корневой директории сайта. root и alias работают по-разному:

root /var/www/alpha.test/html; + location /images/ { ... } = путь к файлу /var/www/alpha.test/html/images/logo.jpg

alias /var/www/shared-assets/images/; + location /images/ { ... } = путь к файлу /var/www/shared-assets/images/logo.jpg
```
Настройка закончена, если все заработает будет магия. ✨🦄🌈

### 4. Активация виртуальных хостов и настройка hosts

#### Создаем симлинки из sites-available в sites-enabled, чтобы Nginx их загрузил:
```
sudo ln -s /etc/nginx/sites-available/alpha.test /etc/nginx/sites-enabled/
sudo ln -s /etc/nginx/sites-available/beta.test /etc/nginx/sites-enabled/
```
<img width="1336" height="89" alt="image" src="https://github.com/user-attachments/assets/c5f49931-d7cc-4dc3-b205-c7a709a145f7" />

#### Настройка hosts на компьютере:
Этот шаг я уже сделал в самом начале, так что теперь браузер понимает, что alpha.test — это сервер.

🚀

### 5. Проверка конфигурации и перезапуск Nginx

*Deepseek сказал, что это важнейший шаг, я ему верю.*
#### Проверка конфига на ошибки перед перезагрузкой:
```
sudo nginx -t
```
<img width="964" height="100" alt="image" src="https://github.com/user-attachments/assets/06836be9-dec2-43b4-bf70-587c36910c36" />

Все работает успешно! 🥳

#### Теперь перезапускаем Nginx, чтобы применить изменения:
```
sudo systemctl restart nginx
```

### 6. Тестирование

#### Откроем браузер и перейдем по адресам:

* ```http://alpha.test``` — должно автоматически перебросить на ```https://alpha.test```.

Тут я почти расплакался 😢, потому что сайт не запустился, но быстро пофиксил: 

1) Была ошибка - dnserror, следовательно скорее всего что-то не так с hosts.
2) Либо я просто забыл сохранить их, либо еще что-то.
3) Изменил файл ```C:\Windows\System32\drivers\etc\hosts```:
<img width="1136" height="678" alt="image" src="https://github.com/user-attachments/assets/41737320-8087-43c0-aac5-c1f9e8a8bc7c" />
4) Сохранил.
5) Проверил что сохранил.
6) Проверил что сайт открывается и автоматически перебрасывает на ```https://alpha.test```.

<img width="1845" height="291" alt="image" src="https://github.com/user-attachments/assets/c25b937d-3717-4b68-8d1d-1d0a8d6c28d1" />

Браузер ругался на самоподписанный сертификат, но мы конечно же приняли исключение для безопасности и продолжили. 

* ```http://beta.test``` — должно автоматически перебросить на ```https://beta.test```.
1)
<img width="1622" height="268" alt="image" src="https://github.com/user-attachments/assets/1f4f9c07-1e44-42b5-beb4-033cbfcfe40e" />
2)
<img width="1853" height="973" alt="image" src="https://github.com/user-attachments/assets/5879cf72-0996-43fd-93ba-652148dba928" />
3)
<img width="1831" height="336" alt="image" src="https://github.com/user-attachments/assets/8fef1289-9014-4051-8d47-c80ab6dc4b28" />
4)
<img width="1847" height="378" alt="image" src="https://github.com/user-attachments/assets/e5c4b5d8-2b3a-45d9-92c4-07502d302d50" />

#### Проверим редирект с HTTP на HTTPS через командную строку:
```
curl -I http://alpha.test
```
<img width="866" height="234" alt="image" src="https://github.com/user-attachments/assets/01c3c3c8-f70d-4cff-87a6-83c808d11746" />
Все работает. ^_____^

Проверим также логи, чтобы убедиться, что запросы идут в правильные файлы:
```
sudo tail -f /var/www/alpha.test/logs/access.log
```
<img width="1915" height="290" alt="image" src="https://github.com/user-attachments/assets/7422d56e-720e-490c-b16b-6ce095c3c777" />
При обновление страницы появляются новые логи, все правильно.


